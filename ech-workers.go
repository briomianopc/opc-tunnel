...
var (
	...
	// 连接池 (WebSocket 模式)
	wsPool     chan *websocket.Conn
	poolSize   int
	poolActive int64 // 活跃连接数
)
...
func main() {
	...
	if !fallback {
		...
		if err := prepareECH(); err != nil {
			if protoMode == TransportGRPC {
				// gRPC 模式下 ECH 获取失败不是致命错误，可以回退到普通 TLS
				log.Printf("[警告] ECH 配置获取失败，gRPC 将使用普通 TLS: %v", err)
				useECH = false
			} else {
	...
	// 初始化传输层
	InitTransport(protoMode, serverAddr, serverIP, token, useECH)
...
func initConnectionPool() {
	...
	wsPool = make(chan *websocket.Conn, poolSize*2) // 缓冲区是池大小的2倍

	// 预建立连接
	log.Printf("[连接池] 正在预建立 %d 个连接...", poolSize)
	...
	for i := 0; i < poolSize; i++ {
		...
		go func() {
			defer wg.Done()
			conn, err := dialWebSocketWithECH(2)
			if err != nil {
				log.Printf("[连接池] 预建立连接失败: %v", err)
				return
			}
			wsPool <- conn
	...
	wg.Wait()
	log.Printf("[连接池] 预建立完成，成功 %d 个", successCount)

	// 启动连接池维护协程
	go maintainPool()
}

// maintainPool 维护连接池，确保有足够的空闲连接
func maintainPool() {
	ticker := time.NewTicker(5 * time.Second)
	defer ticker.Stop()

	for range ticker.C {
		currentPool := len(wsPool)
		active := atomic.LoadInt64(&poolActive)

		// 如果池中连接不足且活跃连接数不太高，补充连接
		if currentPool < poolSize/2 && active < int64(poolSize*2) {
			go func() {
				conn, err := dialWebSocketWithECH(2)
				if err != nil {
					return
...
// getPooledConnection 从池中获取连接，如果池为空则新建
func getPooledConnection() (*websocket.Conn, error) {
	atomic.AddInt64(&poolActive, 1)
...
// returnToPool 将连接归还到池中（如果连接仍有效）
func returnToPool(conn *websocket.Conn) {
	atomic.AddInt64(&poolActive, -1)
...
func runProxyServer(addr string) {
	...
	if numConns > 1 {
		log.Printf("[代理] 连接池大小: %d", poolSize)
	}
...
func initNetworkStack() error {
	tunEndpoint = channel.New(512, 1500, "")
